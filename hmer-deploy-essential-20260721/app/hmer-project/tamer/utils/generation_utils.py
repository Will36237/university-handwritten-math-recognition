...
class DecodeModel(pl.LightningModule):
    ...
    def _structure_scores(
        ...
        _, sim = self.transform(
            [src[0].repeat(2, 1, 1, 1)],
            [src_mask[0].repeat(2, 1, 1)],
            tgt,
        )
        struct_out, illegal = to_struct_output(hyps, self.device)
        length = struct_out.shape[1]
        struct_loss = ce_loss(
            sim, struct_out, ignore_idx=-1, reduction="none"
        )
        struct_loss = rearrange(struct_loss, "(b l) -> b l", l=length)
        mask = struct_out == -1
        token_count = (~mask).sum(dim=1)
        struct_loss = -torch.sum(struct_loss, dim=-1) / token_count
        struct_loss = rearrange(
            struct_loss, "(n b) -> n b", n=2
        ).mean(dim=0)
        return struct_loss, illegal
    ...
    def beam_search(
        ...
        struct_scores, illegal = self._structure_scores(src, src_mask, hyps)
        scores = scores + rev_scores + struct_scores
