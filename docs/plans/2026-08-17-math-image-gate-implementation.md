...
**Architecture:** Reuse the Qwen3.5-2B base model already loaded by the Uni-MuMER PEFT worker. Run a short deterministic three-label classification with the LoRA adapter disabled, then run the existing OCR generation only for `MATH`; propagate stable errors through the unchanged gateway contract to Android.
...
- Reuse the loaded Qwen3.5 model; do not add another model, container, or network service.
...
**Interfaces:**
- Produces: `MathImageDecision`, `parse_math_image_decision(output: str) -> MathImageDecision`, and a gated `UniMumerLoraAdapter.predict(image: Image.Image) -> str`.
...
- [ ] **Step 5: Write failing adapter orchestration tests**

Use fake model/torch contexts and a patched generation method to prove:
...
- [ ] **Step 7: Implement shared-model classification and OCR**

Refactor generation into one private helper. Hold the existing lock across both generations, wrap classification in `with self._model.disable_adapter():`, use deterministic generation with a short classification output limit, and run the existing LoRA prompt only after a `MATH` decision.
